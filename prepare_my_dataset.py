...
def organize_current_data():
    """Organizes your current data structure"""
    ...
    processed_dir = Path("datasets/processed/speed_signs")

    for split in ['train', 'val', 'test']:
        for class_name in list(class_mapping.values()) + ['other_signs']:
    ...
    for speed_dir in raw_positive.iterdir():
        if speed_dir.is_dir() and speed_dir.name in class_mapping:
            ...
            print(f" {new_class_name}: {len(train_files)} train, {len(val_files)} val, {len(test_files)} test")
    ...
    if raw_negative.exists():
        ...
        if negative_files:
            ...
            for split_name, files in [('train', train_neg), ('val', val_neg), ('test', test_neg)]:
                if not files:
                    continue

                target_dir = processed_dir / split_name / 'other_signs'
            ...
            print(f" other_signs: {len(train_neg)} train, {len(val_neg)} val, {len(test_neg)} test")
