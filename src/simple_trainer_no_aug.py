"""
YOLO Trainer for Speed Limit Recognition - NO AUGMENTATION VERSION
"""
...
class SimpleYOLOTrainer:
    ...
    def __init__(self, model_name="yolov8m.pt"):
        ...
        self.project_name = 'speed_limit_no_aug'
    ...
    def train_model(self, data_config="datasets/yolo_detection/data.yaml", epochs=300):
        logger.info(f"Starting BASELINE training (NO AUGMENTATION): config={data_config}, epochs={epochs}")
        ...
        try:
            ...
            results = model.train(
                ...
                # ZERO AUGMENTATION (BASELINE SETTINGS)
                # Photometric (Color)
                hsv_h=0.0,
                hsv_s=0.0,
                hsv_v=0.0,
    ...
    @staticmethod
    def validate_model(model_path, data_config="datasets/yolo_detection/data.yaml", project_name='speed_limit_no_aug'):
        try:
    ...
    def generate_summary(self, val_results=None):
        ...
        summary = []
        summary.append("=" * 80)
        summary.append("TRAINING SUMMARY - BASELINE (NO AUGMENTATION)")
        summary.append("=" * 80)
        ...
        summary.append("  - ALL AUGMENTATIONS DISABLED (Values set to 0.0)")
        summary.append("  - Mosaic: OFF")
        summary.append("  - Mixup: OFF")
        summary.append("  - HSV/Geometric: OFF")
        summary.append("")
        ...
        summary.append(f"  models/{self.project_name}/weights/best.pt")
        summary.append(f"  models/{self.project_name}/weights/last.pt")
        summary.append("")
        summary.append("Plots and Logs:")
        summary.append(f"  models/{self.project_name}/")
        summary.append("=" * 80)
...
def main():
    ...
    detailed_log_path = log_dir / f"training_no_aug_detailed_{timestamp}.log"
    summary_log_path = log_dir / f"training_no_aug_summary_{timestamp}.txt"
    ...
    try:
        ...
        best_model_path = Path(f"models/{trainer.project_name}/weights/best.pt")

        val_results = None
        if best_model_path.exists():
            val_results = trainer.validate_model(str(best_model_path), project_name=trainer.project_name)
